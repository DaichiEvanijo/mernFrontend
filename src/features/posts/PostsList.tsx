...
const PostsList = () => {
  ...
  const performFiltering = useCallback(() => {
    const searchFilteredPosts = orderedPosts.filter(
      (post) =>
        post.title.toLowerCase().includes(searchText.toLowerCase().trim()) ||
        post.author.toLowerCase().includes(searchText.toLowerCase().trim())
    );
  ...
  const currentPosts = useMemo(
    () => filteredPosts.slice(firstPostsIndex, lastPostsIndex),
    [filteredPosts, firstPostsIndex, lastPostsIndex]
  );
  ...
  if (postStatus === "loading") {
    ...
    const postsContent = currentPosts.map((post) => (
      <PostsExcerpt key={post._id} post={post} />
    ));
    content = postsContent.length ? (
      postsContent
    ) : (
      <p style={{ textAlign: "center" }}>
        There is no post to display based on your searching words
      </p>
    );
    ...
    <section className="home">
      <section className="postslist">
        ...
        <div className="filtersection">
          <input
            type="text"
            placeholder="input a prefecture/author"
            value={searchText}
